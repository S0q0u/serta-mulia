...
async function predictClassification(model, image) {
  try {
    const tensor = tf.node
      .decodeJpeg(image) //decode terhadap input baru
      .resizeNearestNeighbor([224, 224]) //resize gambar
      .expandDims() //menambah dimensi gabar
      .toFloat(); //mengubah data yang diproses menjadi float
